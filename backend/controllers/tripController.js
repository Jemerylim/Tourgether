...
exports.createTrip = async (req, res) => {
  ...
  try {
    ...
    const emailPromises = validMembers.map((member) => {
      const invitationLink = `http://52.44.156.98:3000/trip/${trip._id}/accept?userId=${member._id}`;
      const mailOptions = {
...
exports.sendInviteToMember = async (req, res) => {
  ...
  try {
    ...
    const invitationLink = `http://52.44.156.98:3000/trip/${trip._id}/accept?userId=${user._id}`;
    const mailOptions = {
      from: process.env.EMAIL_USER,
